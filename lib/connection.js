...
module.exports = (function() {
	...
	Connection.prototype.sentence = function sentence(data, more) {
		...
		if (_(this).currentReply === '!fatal') { // our last message was a fatal error.
			...
			// If the payload is trapped and gets done, anything after that will
			// be related to another channel, should be safe to push the data.
			else if(_(this).currentProgress === '!done' && _(this).traps[tagChannelId]){
				_(this).packet = _(this).buffer; // backup up the packet
				_(this).buffer = [];
				if (_(this).channels[tagChannelId]) {
					_(this).channels[tagChannelId]._done(_(this).packet, _(this).traps[tagChannelId]);
			...
			_(this).packet = _(this).buffer;
			_(this).buffer = [];
			// There is nothing after a =ret= related to the same channel,
			// should be safe to return the data directly.
			// If there is more data, we should expect another .tag to assume
			// the currentChannelId before it finishes again.
			// This will work assuming that all communications with the
			// routeros uses channels after logging in.
	...
	Connection.prototype.openChannel = function openChannel(id) {
		...
		// Moved this to the 'close' function on the Channel class
		// Should prevent from being triggered before actualy closing
	...
	* @public
	*/
	Connection.prototype.closeAllChannels = function closeAllChannels(){
		var _this = this;
	...
	Connection.prototype.close = function close(force) {
		...
		if (!_(this).connected) {
			...
			_(this).connected = false;
			// this.removeAllListeners(); // Was preventing calling the callback when close was triggered
