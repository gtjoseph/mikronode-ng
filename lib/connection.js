...
module.exports = (function() {
	...
	Connection.prototype.sentence = function sentence(data, more) {
        ...
        if(!more && _(this).currentReply === '!fatal'){
			debugSentence('Sentence2: fatal received, closing.');
		...
		if(/^\.tag=/.test(data)){
			_(this).currentChannelId = data.substring(5);
		}

		else if(/^!/.test(data)){
			if(_(this).currentReply !== null){
				this._push();
			}
	...
	/**
	 * Push the data piece to the channel array,
	 * triggered by the 'sentence' function
	 * @private
	 * @this mikronode-ng.Connection
	 */
	Connection.prototype._push = function _push(){
		if(_(this).channels[_(this).currentChannelId]){
			debugSentence('Sentence2: data sentence for channel %s', _(this).currentChannelId);
            _(this).packet = _(this).buffer;
			_(this).channels[_(this).currentChannelId]._data(_(this).packet);
		}
        _(this).buffer = [];
        _(this).currentChannelId = null;
        _(this).currentReply = null;
	};
			...
			try {
				chan.write(data, parameters, function() {
					...
					chan.on('trap', function(err) {
						...
						chan.close();
						var parsed = Connection.parseItems(err);
						reject(parsed);
					});
