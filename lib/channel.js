...
module.exports = (function() {
	...
	var debugChannel = dbg('mikronode-ng:channel');
	var debugChannelData = dbg('mikronode-ng:channel:data');
	...
	function Channel(id, conn) {
		 ...
		 * @private
		 * @instance
		 * @member {boolean} gottrapped
		 * @memberof mikronode-ng.Channel
		 */
		_(this).gotTrapped = false;
	...
	Channel.prototype._done = function _done(data, trap) {
		if(_(this).gotTrapped){
			debugChannel('Channel %s caught a trap.', _(this).id);
			this.emit('trap', _(this).packet, this);
		}else{
			debugChannel('Channel %s done signal.', _(this).id);
	...
	Channel.prototype._data = function _data(data) {
		...
		var reply = data.shift(); // take the reply type, which is always the first item

		// Prevent some garbage getting pushed to the packet
		if(_(this).saveBuffer && data.length > 0){
		...
		switch (reply){
            ...
            case '!done':
				...
				break;
			case '!trap':
				_(this).gotTrapped = true;
				break;
			case '!fatal':
				this.emit('fatal', _(this).packet, this);
				_(this).packet = [];
				if (!_(this).connection.closing){
					 _(this).connection.close();
				}
				break;
			default:
				// we shouldn't expect anything other than those above
				break;
		}
	};
